...
fn healthy_node_joins_only_after_quorum_and_starts_probationary() {
    ...
    assert!(
        outcome.admitted,
        "quorum should admit a healthy candidate: {outcome:?}"
    );
...
fn tampered_candidate_is_refused_and_not_added() {
    ...
    assert!(
        !outcome.admitted,
        "a divergent measured state must be refused"
    );
    assert!(
        outcome
            .reject_reasons
            .contains(&AdmissionReason::AttestationFailed),
        "reasons: {:?}",
...
fn duplicate_identity_is_flagged() {
    ...
    assert!(
        second
            .reject_reasons
            .contains(&AdmissionReason::DuplicateIdentity),
        "reasons: {:?}",
...
fn probationary_node_cannot_vote() {
    ...
    assert!(
        mesh.is_eligible_voter(ids[1]),
        "established founder may vote"
    );
    assert!(
