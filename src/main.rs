...
use app::{
    ...
    SecretCommand, TemplateCommand, TopCommand, VtpmCommand, VtpmCredentialCommand,
    WorkspaceCommand,
};
...
fn generate_completions(shell: &str) -> anyhow::Result<()> {
    ...
    clap_complete::generate(shell, &mut cmd, "citadel", &mut std::io::stdout());
    Ok(())
}
...
fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        Some(TopCommand::Cluster { command }) => commands::cluster::run(command),
        Some(TopCommand::Tpm { command }) => match command {
                ...
                match cmd {
                    ...
                    Command::Pcr(pcr_cmd) => match pcr_cmd {
                        ...
                        PcrCommand::Baseline(bl_cmd) => match bl_cmd {
                            ...
                            PcrBaselineCommand::Diff { name } => commands::pcr::baseline_diff(
                                &store,
                                backend.as_ref(),
                                &name,
                                format,
                    ...
                    Command::Measure(measure_cmd) => match measure_cmd {
                        ...
                        MeasureCommand::Enroll {
                            bank,
                            pcr,
                            verify_ima,
                        } => commands::measure::enroll(
                            &store_path,
                            backend.as_ref(),
                            &bank,
                            pcr,
