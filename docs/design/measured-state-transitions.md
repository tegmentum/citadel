...
### 10.3 Boot profiles + fleet quorum promotion (Layer 4 — built)

> **Status: built.** Profiles + assignment (`reference.rs` `BootProfile`;
> `node.rs` `define_profile`/`assign_profile`/`appraisal_for`): a verifier holds
> named profiles and appraises each subject against the one assigned to it
> (`node_id → profile`). Quorum promotion (`promotion.rs`
> `PromotionProposal`/`PromotionVote`/`decide_promotion`; `node.rs`
> `propose_promotion`/`vote_on_promotion`/`adopt_promoted_state`; harness
> `promote_state`): a new state runs `unknown → observed → staged →
> quorum-accepted → fleet-accepted`, with eligible peers each judging
> provenance against their own fleet policy — no central "known good". Tests in
> `reference_transition.rs`.
...
* **Layer 4 — boot profiles, quorum promotion, event-log semantic validation
  (§§10.3–10.4). ◑ Mostly built.** Boot profiles + `node_id → profile`
  assignment with per-subject appraisal **built** (`BootProfile`,
  `appraisal_for`); fleet **quorum promotion** built (`promotion.rs`,
  `promote_state`). Remaining: **only** TCG event-log ingestion + replay +
  per-artifact semantic policy — the headline §10.4 piece, **blocked** on
  event-log ingestion (the §5/§6 gap in `distributed-log-shipping-lthash.md`),
  and the unification with the MMA PolicyAuthorize ceremony (§9).
